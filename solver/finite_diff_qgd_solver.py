...
class FiniteDifferenceQGDSolver(Solver):
    def __init__(self, maxeval, eta=0.25, momentum=0.25):
        super().__init__()
        self.optimiser = FiniteDifferenceQGDOptimizer(disp=True, maxiter=maxeval, eta=eta, momentum=momentum, tol=0,
                                                      param_tol=0)


# The code below has been modified from the original Qiskit implementation to use finite difference (rather than
# parameter shift rule) to evaluate gradients.
...
class FiniteDifferenceQGDOptimizer(Optimizer):
    """Quantum Gradient Descent with Epochs optimizer. Modified version of the Qiskit AQGD (Analytic quantum gradient
    descent to use finite difference scheme to evaluate gradients.

    Performs gradient descent optimization with a momentum term and customized step length schedule for parametrized
    quantum gates.
    """
    ...
    def _compute_objective_fn_and_gradient(self, params: List[float],
        ...
        param_sets_to_eval = params + np.concatenate(
            (np.zeros((1, num_params)),  # copy of the parameters as is
             ...
             -np.eye(num_params) * eps),  # copy of the parameters with the negative shift
            axis=0)
        # Evaluate,
        ...
        gradient = (values[1:num_params + 1] - values[1 + num_params:]) / (2. * eps)
