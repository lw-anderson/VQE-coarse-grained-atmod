...
class VatanAnsatz(QuantumAnsatz):
    """ 
    Ansatz for real functions in one dimension, based on SO(4) circuit from Vatan and Williams Phys. Rev. A 69, 032315
    (2004).
    """

    def __init__(self, num_qubits: int, depth: int, cyclic=False, interact_first=False, offset=1, coupling_range=1,
                 flag=True):
        ...
        if not interact_first:
            ...
            num_params = 3 * self.num_a + 6 * ((self.num_a_cols - 1) * self.num_a + self.num_b_cols * self.num_b)

        else:

            self.num_a_cols = (self.depth // 2)
            self.num_b_cols = self.depth // 2 + (self.depth % 2)
            num_params = 6 * (self.num_a_cols * self.num_a + self.num_b_cols * self.num_b)
    ...
    def _create_circuit(self):
        ...
        # consistency check
        if not self.interact_first:
            assert (ind - self.num_a) * 6 + self.num_a * 3 == len(self.params), \
                ValueError("Something went wrong!")
        else:
            assert ind * 6 == len(self.params), \
                ValueError("Something went wrong!")
    ...
    def get_operator(self, ind):
        ...
        ct.rz(local_params[2], 0)

        if ind >= self.num_a or self.interact_first:
            ct.rz(local_params[3], 1)
            ct.ry(local_params[4], 1)
            ct.rz(local_params[5], 1)
    ...
    def get_local_params(self, ind):
        if ind < self.num_a and not self.interact_first:
            local_params = self.params[3 * ind:3 * (ind + 1)]
        else:
            if not self.interact_first:
                ind2 = ind - self.num_a
                offset = 3 * self.num_a
                ...
                ind2 = ind
                offset = 0
            local_params = self.params[(offset + 6 * ind2):(offset + 6 * (ind2 + 1))]
        return local_params

    def get_initial(self, initial_state='equal'):
        """
        calculate the intial state corresponding to a constant function.
        """
        ...
        if initial_state == 'equal':
            ...
            initial_values = np.zeros_like(len(self.params))
