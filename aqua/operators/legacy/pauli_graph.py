# This file has been changed from the original qiskit aqua. Changes are
# made are highlighted in code.
...
class PauliGraph:
    ...
    def _create_edges(self):
        ...
        # LEWIS : CHANGED ONE LINE TO REMOVE - and i coeffs from nodes (which don't affect commutation)
        # before:  a = np.array([[conv[e] for e in reversed(n.to_label())] for n in self.nodes], dtype=np.int8)
        a = np.array([[conv[e] for e in reversed(n.to_label().replace('i','').replace('-',''))] for n in self.nodes],
                     dtype=np.int8)
        b = a[:, None]
            ...
            for i, groupi in enumerate(gp):
                ...
                for _, p in groupi:
                    for k in range(self._nqbits):
                        if p.z[k] or p.x[k]:
                            header[1]._z[0, k] = p.z[k]  # LEWIS: CHANGED TO z SETTER FOR UPDATED Pauli OBJECT
                            header[1]._x[0, k] = p.x[k]  # LEWIS: CHANGED TO x SETTER FOR UPDATED Pauli OBJECT
