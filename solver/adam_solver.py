...
# The class defined below is a modified version of the Qiskit ADAM optimiser. It has been combined with parts of the
# Qiskit AQGD optimiser such that objective function and gradients are calculated using a single call to the
# cost function.
...
class ADAM(Optimizer):
    ...
    def _compute_objective_fn_and_gradient(self, params: List[float],
        ...
        (obj_value, stdev) = values[0]

        # return the gradient values
        gradient = 0.5 * (values[1:num_params + 1, 0] - values[1 + num_params:, 0])
        return (obj_value, stdev), gradient
    ...
    def minimize(self, objective_function: Callable[[np.ndarray], float], initial_point: np.ndarray) \
        ...
        (obj_value, stdev), derivative = self._compute_objective_fn_and_gradient(initial_point, objective_function)

        # Additional changes to terminate with variance condition
        ...
        while self._t < self._maxiter:  # and (self._t < rolling_variance_steps or rolling_var > stdev ** 2):
            rolling_var = np.var(expectation_values[max(0, self._t - rolling_variance_steps): max(0, self._t)])
            if self._t > 0:
                (obj_value, stdev), derivative = self._compute_objective_fn_and_gradient(params, objective_function)
            expectation_values.append(obj_value)
            print(" Iter: {:4d} | Obj: {:11.6f} | Grad Norm: {:f}"
                  .format(self._t, obj_value, np.linalg.norm(derivative, ord=np.inf)))
            self._t += 1
...
class ADAMSolver(Solver):
    def __init__(self, maxeval, lr):
        super().__init__()
        self.optimiser = ADAM(maxiter=maxeval, lr=lr, tol=0)
