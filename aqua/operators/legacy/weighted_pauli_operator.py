...
from aqua import AquaError, aqua_globals, MissingOptionalLibraryError
...
class WeightedPauliOperator(LegacyBaseOperator):
    ...
    def to_opflow(self, reverse_endianness=False):
        ...
        from aqua.operators import PrimitiveOp
    ...
    def _scaling_weight(self, scaling_factor, copy=False):
        ...
        """
        if not isinstance(scaling_factor, (int, float, complex, np.int, np.float, np.complex_)):
            raise ValueError(
    ...
    def __rmul__(self, other):
        """ Overload other * self """
        if isinstance(other, (int, float, complex, np.int, np.float, np.complex_)):
    ...
    def __mul__(self, other):
        """ Overload self * other """
        if isinstance(other, (int, float, complex, np.int_, np.float_, np.complex_)): # LEWIS: updated numpy types
            return self._scaling_weight(other, copy=True)
        else:
    ...
    def construct_evaluation_circuit(self, wave_function, statevector_mode,
        ...
        if self.is_empty():
            raise AquaError("Operator is empty, check the operator.")
        # pylint: disable=import-outside-toplevel
        from aqua.utils.run_circuits import find_regs_by_name
...
class Z2Symmetries:
    ...
    def find_Z2_symmetries(cls, operator) -> 'Z2Symmetries':  # pylint: disable=invalid-name
        ...
        for pauli in operator.paulis:
            stacked_paulis.append(np.concatenate((pauli[1].x, pauli[1].z), axis=0).astype(np.int_))
